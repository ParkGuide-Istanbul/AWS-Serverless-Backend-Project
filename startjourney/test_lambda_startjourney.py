...
def test_successful_start_journey():
    login_event = {
        "body":   {
            "username": "aliçakır90",
            "password": "çakırali24",
    ...
    assert response['statusCode'] == 200, "journey başlatma testi başarısız"
    event = {
        "headers": {"authorization-token": response_token},
    }
...
def test_already_have_active_journey():
    login_event = {
        "body":   {
            "username": "aliçakır90",
            "password": "çakırali24",
    ...
    response = startjourney_lambda_handler(event, None)
    assert response['statusCode'] == 400, "aktif journey si olan kullanıcının yeni journey başlatma testi başarısız"
